...
from tools import ObjVal, SolVal, newSolver, pairs

def solve_model(Teams,(nbIntra,nbInter,nbPerWeek,nbWeeks)):
  nbTeams,nbDiv = sum([1 for sub in Teams for e in sub]),len(Teams)
  ...
  Value=[w*x[i][j][w] for Div in Teams for i in Div for j in Div for w in range(nbWeeks) if i<j]
...
def add_objective(s,Teams,nbWeeks,x):
  Value=[w*x[i][j][w] for Div in Teams for i in Div for j in Div for w in range(nbWeeks) if i<j]
  return Value

def add_constraints(s,Teams,nbTeams,nbWeeks,nbPerWeek,nbIntra,nbDiv,nbInter,cuts,x):
    ...
    Value = add_objective(s,Teams,nbWeeks,x)
    s.Maximize(s.Sum(Value))

def solve_model_big(Teams,(nbIntra,nbInter,nbPerWeek,nbWeeks)):
  nbTeams,nbDiv,cuts,iter = sum([1 for sub in Teams for e in sub]),len(Teams),[],0
  while iter < 10:
    s = newSolver('Sports schedule', False)
    x = [[[s.NumVar(0,1,'') if i<j else None for _ in range(nbWeeks)] 
              for j in range(nbTeams)] for i in range(nbTeams-1)]
    add_constraints(s,Teams,nbTeams,nbWeeks,nbPerWeek,nbIntra,nbDiv,nbInter,cuts,x)
    rc = s.Solve()
    if nbPerWeek == 1:
      for w in range(nbWeeks):
        for i in range(nbTeams-2):
          for j in range(i+1,nbTeams-1):
            for k in range(j+1,nbTeams):
              if sum([SolVal(x[p[0]][p[1]][w]) for p in pairs([i,j,k],[])]) > 1:
                cuts.append([[i,j,k],[w,1]])
                for l in range(k+1,nbTeams):
                  for m in range(l+1, nbTeams):
                    if sum([SolVal(x[p[0]][p[1]][w]) for p in pairs([i,j,k,l,m],[])]) > 2:
                      cuts.append([[i,j,k,l,m],[w,2]])
  s = newSolver('Sports schedule', True)
  x = [[[s.IntVar(0,1,'') if i<j else None for _ in range(nbWeeks)] 
        for j in range(nbTeams)] for i in range(nbTeams-1)]
  add_constraints(s,Teams,nbTeams,nbWeeks,nbPerWeek,nbIntra,nbDiv,nbInter,cuts,x)
